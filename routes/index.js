...
router.post('/lf_primary_add', multer({ storage: storage3 }).single('file'), function (req, res, next) {
  ...
  sql = 'insert into param_requirement (subject,filename,file_addr,upload_date,status,note,timeline_type) values (\'' + req.body.subject + '\',\'' + req.body.filename + '\',\'' + req.file.path+ '\',\'' + upftime + '\',\'' + req.body.status + '\',\'' + req.body.note + '\',\'timeline\')';
